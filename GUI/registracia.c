...
int main()
{
    ...
    win_body = newwin(14, 80, 0, 0);
    assert(win_body != NULL);
    box(win_body, 0, 0);
    win_form = derwin(win_body, 10, 78, 3, 1);
    assert(win_form != NULL);
    box(win_form, 0, 0);
    mvwprintw(win_body, 1, 1, "F1 to quit, F2 to print fields content, SIGN IN");

    fields[0] = new_field(1, 10, 0, 5, 0, 0);
    fields[1] = new_field(1, 40, 0, 15, 0, 0);
    fields[2] = new_field(1, 10, 2, 5, 0, 0);
    ...
    set_form_sub(form, derwin(win_form, 8, 76, 1, 1));
    post_form(form);
